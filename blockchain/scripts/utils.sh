...
setGlobalsCLI() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    export CORE_PEER_ADDRESS=peer0.administration.scheisse.edu:7051
  elif [ $USING_ORG -eq 2 ]; then
    export CORE_PEER_ADDRESS=peer0.students.scheisse.edu:9051
  elif [ $USING_ORG -eq 3 ]; then
    export CORE_PEER_ADDRESS=peer0.org3.scheisse.edu:11051
  else
    errorln "ORG Unknown"
  fi
}

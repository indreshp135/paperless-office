...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o orderer.scheisse.edu:7050 --ordererTLSHostnameOverride orderer.scheisse.edu -c ${CHANNEL_NAME} --tls --cafile $ORDERER_CA
  { set +x; } 2>/dev/null
